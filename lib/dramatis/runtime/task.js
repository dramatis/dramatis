...
(function(){
  ...
  var Task = Runtime.Task =
    new Dramatis.Runtime.Class(function Task(callable, args, continuation, id) {
      ...
      call: function() {
        ...
        var exception;
        try {
          // console.debug("call",this.id);
          value = this.callable.apply(this.args);
        } catch(e) {
          // global.console.debug("caught task ex "+e);
          // global.console.debug("caught task ex "+e.stack);
          exception = e;
        }
        ...
        // ... for better or worse
        if (exception) {
          if (exception instanceof Dramatis.Exception.Raise) {
