...
(function(){
  ...
  Remote.fromJSON = function(args) {
    var route = Runtime.Reactor.parse_url(args[0]);
    var id = args[1];
    var name = Dramatis.Director.current.lookup(route, id);
    if (!name) {
      name = new Remote(route, id);
    }
    // console.debug("XX",name,name.constructor+"");
    return name;
  };
