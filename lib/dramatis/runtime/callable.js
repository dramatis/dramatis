...
(function(){
  ...
  var Callable = Runtime.Callable =
    new Dramatis.Runtime.Class(function Callable(/*...*/){
      ...
      } else if (pair[0] instanceof Dramatis.Actor) {
        pair[0] = pair[0].__name__.__runtime__;
      } else if (pair[0].__dramatis__) {
        pair[0] = pair[0].__dramatis__.name().__runtime__;
      }
      ...
      apply: function(args) {
        var target = this.name.actor();
        if (this.type !== "actor") {
          target = target.behavior;
        }
        var fn = this.fn(target);
        return fn.apply(target, args);
      },
      raise: function raise(exception) {
        // console.debug("raise",$.print(exception));
        if (!(exception instanceof Dramatis.Exception)) {
          exception = new Dramatis.Exception.UnknownException(exception);
  ...
  Callable.fromJSON = function(args) {
    Dramatis.revive(args);
    var Cls = this;
    var fn = new Cls([args[0], args[1]]);
    if (args[2]) {
      fn.type = args[2];
    }
    return fn;
